...
# 整合之后的思想就是：将所有的情况各写一个类，并将其判断方法test，执行方法act，以及这些
# 类中会共用到的一些方法，比如一些具体执行方法写到基类之中。然后情况类继承这个基类。
# 最主要的类RequestHandler类，根据请求消息进行判断处理。主要使用BaseHTTPRequestHandler能够获得的
# 相对请求路径self.path，进行判断进行执行。
